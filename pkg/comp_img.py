...
def runAllImageSimilaryFun(para1, para2, isfile=True, isprint=True):
    ...
    # c1,c2  11 29 17    0.30 0.31
    if isfile:
        img1, img2 = cv2.imread(para1), cv2.imread(para2)
    else:
        img1, img2 = para1, para2
    hash1 = aHash(img1)
    hash2 = aHash(img2)
    n1 = cmpHash(hash1, hash2)
    # print('均值哈希算法相似度aHash：', n1)
    ...
    # print("单通道的直方图", n5)
    # print("%d %d %d %.2f %.2f " % (n1, n2, n3, round(n4[0], 2), n5[0]))
    result = (1 - float(n1 / 64),
              1 - float(n2 / 64),
              1 - float(n3 / 64),
              n4[0] if n4 < 1 else n4,
              n5[0] if n5 < 1 else n5)
    if isprint:
        if isfile:
            print(para1, para2, end=' ')
        print("%.2f %.2f %.2f %.2f %.2f " % result)
    else:
        return result
...
if __name__ == "__main__":
    ...
    import sys
    runAllImageSimilaryFun(sys.argv[1], sys.argv[2])
